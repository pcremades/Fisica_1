...
void setup() {
  ...
  serialPorts = Serial.list(); //Obtener lista de interfaces Serie
  for ( int i=0; i<serialPorts.length; i++) { //Buscar interfaces Serie que correspondan a Arduino
    if ( serialPorts[i].contains("ttyACM") | serialPorts[i].contains("ttyUSB") ) {  //Si la encuentra, intentar abrir
...
void draw() {
  ...
  //Actualiza un buffer circular con la lista de posiciones
  ...
  //Dibuja las posiciones pasadas con un efecto fade out.
...
//----------->Implementar una derivada de orden superior para mejorar el dato.
